...
export const elapsedTime = (serverTimestamp) => {
  ...
  const serverTime = new Date(
    serverTimestamp.seconds * 1000 + serverTimestamp.nanoseconds / 1000000
  );
  const elapsedMilliseconds = Date.now() - serverTime.getTime();

  const timeFrames = [
    { limit: 60 * 1000, label: "másodperce", divisor: 1000 },
    { limit: 60 * 60 * 1000, label: "perce", divisor: 60 * 1000 },
    { limit: 24 * 60 * 60 * 1000, label: "órája", divisor: 60 * 60 * 1000 },
    { limit: 7 * 24 * 60 * 60 * 1000, label: "napja", divisor: 24 * 60 * 60 * 1000 }
  ];
  ...
  // Ha az eltelt idő meghaladja az egy hetet, számítsuk hetekben
